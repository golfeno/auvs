...
class MotorMode(Enum):
    DUAL = auto()    # дифференциал движков (единственный режим; SINGLE убран)
...
class VehicleState:
    ...
    avoid_closest: float = 999.0  # дист. до ближайшей точки сонара, м
    avoid_yaw: float = 0.0        # текущая курсовая поправка обхода, рад
    avoid_dir: str = '-'          # выбранная сторона ухода (L/R/U/D)
    sonar_valid: int = 0          # сколько лучей сонара дали конечную дистанцию
    zL: float = 999.0; zR: float = 999.0; zU: float = 999.0; zD: float = 999.0  # дист. по зонам сонара
    roll_abs: float = 0.0
...
class Phys:
    P_Z0 = 101325.0
    RHO_G = 9810.0
    DT = 0.01   # 50 Гц — совпадает с частотой одометрии (>=30 Гц)
...
class Lim:
    rud_spd = 2.4
    cruise = -22.0  # тяга круиза (вперёд = отрицательная): больше по модулю -> быстрее
    ...
    soft_start_t = 5.0  # с: длительность мягкого старта (плавный разгон до cruise)
    avoid_thrust = -24.0  # тяга в фазе обхода: проходит мимо помехи (терм.~0.4 м/с),
                          # не останавливается; speed_factor в AVOID не применяется
    avoid_turn_gain = 1.3  # множитель отворота обхода ОТНОСИТЕЛЬНО курса входа
                           # (желаемый курс = entry + yaw_offset*gain). Доминирует над целью.
    head_v_full = 0.6  # м/с: при этой (крейсерской) скорости руль на полном авторитете
    head_v_min = 0.12  # мин. доля авторитета руля на околонулевой скорости (старт)
    yd_frac = 0.9     # доля диф. курса от тяги (жёстче держит курс, меньше дуги)
    # --- Фаза 3 (Сближение): стоп → разворот на месте → газ по прямой ---
    r_app = 4.0       # радиус сближения: ближе этого включается фаза 3 (или 2b если Z не готов)
    ...
    turn_thrust = 125.0     # ПОЛНЫЙ дифференциал движков для быстрого разворота на месте
    turn_gain = 120.0       # P по yaw_err (быстро доворачивает)
    turn_damp = 16.0       # D по скорости рыскания (гасит перерегулирование/осцилляцию)
    approach_thrust = 5.0  # ход «по прямой» в фазе сближения (умеренный)
    # Фаза 3 (APPROACH): УСИЛЕННЫЙ разворот движками в режимах глубины РУЛИ/ОБА
    # (там гор.рули заняты глубиной и слабо помогают курсу -> доворачиваем мотором).
    app_turn_thrust = 40.0   # макс. дифференциал движков на развороте в фазе сближения
    app_turn_gain = 30.0     # P по yaw_err в фазе сближения (резче доворот)
    app_turn_damp = 70.0     # D демпфирование разворота в фазе сближения
    turn_first = 0.35   # рад (~20°): раньше разворачивается на месте -> меньше дуги
    # --- LOS-наведение по прямой (метод lookahead-точки) ---
    los_lookahead = 3.0      # Δ упреждения, м: МЕНЬШЕ -> активнее тянет на линию (резче руль)
