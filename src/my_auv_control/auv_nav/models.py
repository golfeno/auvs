...
class Lim:
    ...
    yd_frac = 0.6     # макс. доля дифференциала курса от модуля тяги (чтобы не крутило на месте)
    ...
    align_tol = 0.1745  # ±10° — допуск наведения носа перед газом по прямой
    turn_thrust = 8.0   # дифференциал тяги для разворота на месте (моторы враздрай)
    turn_gain = 12.0    # усиление разворота по yaw_err
