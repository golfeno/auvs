#!/usr/bin/env python3
"""Launch для ТЕСТОВОГО мира (раздел 5 ВКР: проверка обхода/наведения/глубины).

Поднимает Gazebo с worlds/test_world.sdf (блок + стена + цилиндр под сценарии
валидации), мост ros_gz_bridge, виртуальный барометр, статические TF сенсоров и
(опционально) RViz2. Аналогичен simulation.launch.py, но с тестовым миром.

Запуск:
  ros2 launch my_auv_bringup test_world.launch.py
  ros2 launch my_auv_bringup test_world.launch.py rviz:=false
Затем в другом терминале — автопилот с тестовым маршрутом:
  ros2 run my_auv_control autopilot   (укажите файл waypoints_test.txt)
"""
import os
